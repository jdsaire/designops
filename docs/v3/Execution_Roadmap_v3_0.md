...
| 6.2.2 | Multipage migration: three dedicated pages (Capabilities/About/Contact) built from a depth-aware path contract (`core/paths.js` + `data-nav-depth`); capabilities carousel, evolution timeline, and contact organism migrated off Main into their own routes; two depth-blind defects fixed (i18n fetch, navchrome prefix); Main reduced to teasers. Inserted ahead of 6.3 by authorized deviation. ES for this pass's new keys generated inline via `/designops-copy-es` (roadmap ES-debt revoked for this run); **6.3 and 6.4 remain open** | Code · **Opus 4.8** | 6.2.1 merged site + CV inventory + UltraSpec | eight-page routed site | Grep-verifiable parity + served-path resolution across eight pages |
